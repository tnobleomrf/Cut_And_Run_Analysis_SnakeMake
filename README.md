...
The samples.csv file in the config folder is used to indicate sample names, FASTQ location, specify control and treatment, and finally merge your samples if needed. The samples.csv file provided currently has file paths to all the test FASTQ files and other information that will need to be updated with your appropriate samples. The columns of the samples file are; ‘sample’, ‘fastq1’, ‘fastq2’, ‘sampleType’, ‘Control’, and ‘merged_sample’. The first ‘sample’ column is used to specify the sample name, which can be anything that identifies that specific sample. This sample name will also be used to identify the output files. The second and third columns ‘fastq1’ & ‘fastq2’ are used to specify the paired FASTQ files location. The next column, ‘sampleType’ is used to indicate if the sample is a control or a treatment. The next column, ‘Control’ is used to specify the matched control sample for each treatment sample by using the sample name provided in the first column. Individual or multiple controls can be used for each experiment but be sure to put “NA” in the ‘control’ column if the sample is itself a control. Finally, the last column ‘merged_sample’ is used to indicate if a samples’ reads should be added to another sample. This is generally used if an individual sample comes back with more reads at a later time and you want to include them in your analysis. If no sample is being merged, put the same name in the ‘merged_sample’ column that is in the first ‘sample’ column.
